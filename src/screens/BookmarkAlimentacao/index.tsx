...
export function BookmarkAlimentacao({ navigation, route }) {
  ...
  //const [filter, setFilter] = useState([]);
    ...
    const list = async () => {
        try {
            const alimentacaoRef = collection(firestore, "escolherAlimentacao");
            const q = query(alimentacaoRef, where('usuario', '==', codigo));
            const querySnapshot = await getDocs(q);
            let dados: any = [];
            querySnapshot.forEach((doc) => {
                const alimentacao = {
                        id: doc.data().alimentacao,
                        nome: doc.data().nome,
    ...
    const renderItem = ({ item }) => {
        ...
        >
            ...
            <View style={{ width: '65%', paddingHorizontal: 20 }}>
                ...
                <Text style={{ color: COLORS.gray }}>{item.tempo} mins | {item.calorias} kcal </Text>
            </View>
        </TouchableOpacity>
        )
    }
